...
export default function ProjectsSection({ isHomepage = false }: ProjectsSectionProps) {
  ...
  const rawFiltered = isHomepage
    ? allProjects.slice(0, 3)
    : selectedCategory === "All"
    ? allProjects
    : allProjects.filter((p) => p.category === selectedCategory)

  const filteredProjects = isHomepage ? rawFiltered.slice(0, 3) : rawFiltered
  ...
  return (
    <section id="projects" ref={sectionRef} className="border-b border-neutral-200 bg-white py-16 sm:py-24 dark:border-neutral-800 dark:bg-neutral-950 overflow-hidden">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-12">
        ...
        {/* Video Showcase (Only on full /projects page) */}
        {!isHomepage && (
        ...
        {/* Category Filters (Only on full /projects page) */}
        {!isHomepage && (
